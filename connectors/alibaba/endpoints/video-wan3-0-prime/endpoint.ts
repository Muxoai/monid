...
import { zWan30PrimeBody } from "./schema/inputs.ts";

/**
 * Wan 3.0 Prime Video — the all-in-one Wan 3.0 tier: up to 30 seconds from a text prompt,
 ...
 */
const zParameters = zWan30PrimeBody.shape.parameters.unwrap();

export default defineEndpoint({
    meta: {
        displayName: "Wan 3.0 Prime Video",
        summary:
            "Generate up to 30s of video fast from text, frames, reference media, documents, or links.",
        description:
            'Generate video quickly from any mix of inputs: a text prompt (up to 20,000 characters), pinned first/last frames, reference images (up to 10), reference video and audio clips, an office document or PDF, or a web link, with Wan 3.0 Prime — the high-speed tier with capabilities aligned to the standard model. Supports smart duration ("auto"), adaptive aspect ratio, and prompt rewriting. Returns a downloadable MP4 video_url (24h expiry) with synchronized audio, 2-30 seconds, 480P/720P/1080P. Suited for: short-form social clips, ads and product demos, storyboard-to-video, document and article explainers, character-consistent scenes.',
    ...
    endpoint: "/v1/video/wan3.0-prime",
    request: {
        method: "POST",
        ...
        model: {
            ...
            components: {
                "480p": {
                    ...
                    unit: Unit.SECOND,
                    label: "480P seconds",
                    description: "US$0.068 per billed second at 480P",
                    consumes: { credit: "default", amount: 0.068 },
                },
                "720p": {
                    kind: UsageModelKind.PER_UNIT,
                    unit: Unit.SECOND,
                    label: "720P seconds",
                    description: "US$0.14 per billed second at 720P",
                    consumes: { credit: "default", amount: 0.14 },
                },
                "1080p": {
                    kind: UsageModelKind.PER_UNIT,
                    unit: Unit.SECOND,
                    label: "1080P seconds",
                    description: "US$0.28 per billed second at 1080P",
