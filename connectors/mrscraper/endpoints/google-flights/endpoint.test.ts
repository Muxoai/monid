...
const ID = "mrscraper#google/flights";
...
Deno.test(`${ID} happy (synthetic): ten tokens, the caller gets the inner data`, async () => {
...
Deno.test(`${ID}: a round trip needs a return date; codes and dates are gated`, async () => {
    const unit = await testSealedUnit(ID);
    const fixture = await loadFixture(`${fixturesDir}synthetic-happy.json`);
